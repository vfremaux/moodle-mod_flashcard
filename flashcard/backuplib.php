<?PHP //$Id: backuplib.php,v 1.1 2011-10-15 12:25:53 vf Exp $
    ...
    */
    ...
    function flashcard_backup_mods($bf, $preferences) {
        ...
        $flashcards = get_records('flashcard', 'course', $preferences->backup_course, 'id');
        if ($flashcards) {
    ...
    function flashcard_backup_one_mod($bf, $preferences, $flashcard) {
        ...
        if (is_numeric($flashcard)) {
            $flashcard = get_record('flashcard', 'id', $flashcard);
        }
        ...
        fwrite ($bf,full_tag('AUDIOSTART', 4, false, $flashcard->audiostart));
    ...
    */
    function backup_flashcard_deck($bf, $preferences, &$flashcard) {
        ...
        $cards = get_records('flashcard_deckdata', 'flashcardid', $flashcard->id);
        /// If there is card
        if ($cards) {
    ...
    */
    function backup_flashcard_cards($bf, $preferences, &$flashcard) {
        ...
        $cards = get_records('flashcard_card', 'flashcardid', $flashcard->id);
        /// If there is card
        if ($cards) {
